define(["mettle","bbq","Velocity","Velocity.ui"], function(Backbone,bbq,Velocity) {
    ...
    //View =====================================================================
        var ViewConstructor = Backbone.View, ViewProto = Backbone.View.prototype;
        ...
        ViewProto.delegateEvents = _.wrap(ViewProto.delegateEvents, function(original, events) {
            ...
            _.each(events, function(handler, event) {
                var listeners = event.match(/^listenTo\s(.*)/),
                    transitions = event.match(/^transition\s(.*)/);
                if (listeners) this.listenTo(this.globalEventBus, listeners[1], this[handler]);
                if (transitions) {
                    this.transition = this.transition || {};
                    var trans = transitions[1], options = handler;
                    if(trans === 'in' || trans === 'out'){
                        this.transition[trans] = new Backbone.Hg(options);
                    }
                }
            }, this);
        ...
        ViewProto.undelegateEvents = _.wrap(ViewProto.undelegateEvents, function(original, events) {
            ...
            _.each(events, function(event){
                if ( event.match(/listenTo /) ) this.globalEventBus.off( event );
            }, this);
        ...
        Backbone.Hg    = Backbone.Model.extend(_.extend({
            ...
            initialize:     function(options){
                this.listenTo(this,'change:status',this.animationState);
            },
            animationState: function(){ _.log('update: '+this.get('status')); },
            start:          function(view){
                this.set({status:'started',view:view});
                var $el = view.$el, x, def = $.Deferred(), Hg = this, sequence = _.result(this,'sequence');
                if(_.isEmpty(sequence)) {
                    //determine where the view is currently positioned
                    var right = $el.css('right');
                    _.log('right: '+right);
                    if(right === '0px'){
                        x = (Backbone.Forge._direction === 'left')? '100%' : '-100%';
                        this.reverse();
                    } else {
                        x = 0;
                    }
                    Hg.animate(view,{right:x}).then(function(){
            ...
            reverse:        function(){
                switch(Backbone.Forge._direction){
                    case 'left': Backbone.Forge._direction = 'right'; break;
                    case 'right': Backbone.Forge._direction = 'left'; break;
                }
                _.log(Backbone.Forge._direction);
            },
            ...
            animate:        function(view,props,opts,then){
                var def = $.Deferred(), Hg = this, opts = opts || {}, then = (_.isFunction(then))? then : _.noop;
    ...
    //Model ====================================================================
    Backbone.Computed = function (data, name, model) {
        this.name = name;
    ...
    _.extend(Backbone.Computed.prototype, {
    ...
    Backbone.entangle = function (klass, associations) {
    ...
    Backbone.unentangle = function (klass) {
        var proto = klass.prototype;
    ...
    Backbone.Model = Backbone.Model.extend(_.extend({
        ...
        set:             function (key, val, options) {
                ...
                for (attr in attrs) {
                    if (attrs.hasOwnProperty(attr)) {
                        if (attr in computeds) {
                            ...
                            if (firstLoop) computedsAttrs[attr] = attrs[attr];
                            delete attrs[attr];
            ...
            if (!changing) {
                ...
                for (attr in computeds) {
                    if (computeds.hasOwnProperty(attr)) previousComputeds[attr] = computeds[attr].value;
                }
            ...
            for (attr in attrs) {
                if (attrs.hasOwnProperty(attr)) {
                    ...
                    if (!_.isEqual(_.getPath(path, current), val)) {
                        ...
                        for (i = 0; i < escapedPath.length; i++) escapedPath[i] = escapedPath[i].replace(/\./g, '!.');
                        changes.push({ path: path, escapedPath: escapedPath, attr: attr, val: val });
                    }
        ...
        _initComputeds:  function () {
            ...
            for (name in computeds) {
                if (computeds.hasOwnProperty(name)) this.addComputed(name, computeds[name], {silent: true});
            }
            for (name in computeds) {
                if (computeds.hasOwnProperty(name)) this._comps.computeds[name].update();
            }
        },
        addComputeds:    function (key, val, options) {
            ...
            for (name in attrs) {
                if (attrs.hasOwnProperty(name)) {
                    ...
                    computed = this._comps.computeds[name] = new Backbone.Computed(computed, name, this);
                    if (!silent) {
                        computed.update();
                    }
                }
    ...
    _.extend(Forge.prototype, {
        ...
        goto:         function(name){
            var self = this, next = this.cast(name), active = (!_.isNull(this.active))?this.cast(this.active):null;
		    if (!_.isNull(active)) {
                active.transition.out.start(active).then(function(){
                    self.previous = this.id;

                    if(self.addToHistory(this.id)) _.log('trans out >'+ self._history.length);
                });
		    }
            next.render().then(function(){
                this.$el.append(next.$el);
                next.transition.in.start(next).then(function(){
                    self.active = this.id;
                    self.def.resolveWith(this,[this]); _.log(self.active+': trans in');
                });
            });
            return self.def.promise();
        },
        addToHistory:      function(page){
            if(_.contains(this._history,page)){
                return false;
            } else {
                aPush.call(this._history,page);
                return true;
            }
        },
        cast:         function(name){
            var mold = this.foundry.findWhere({ 'name': name });
            if(this.isMold(mold)) return (this.isLayout(mold.get('layout'))) ? mold.get('layout') : null;
        },
